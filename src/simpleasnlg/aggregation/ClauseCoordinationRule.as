...
package simpleasnlg.aggregation
{
	...
	public class ClauseCoordinationRule extends AggregationRule
	{
		 ...
		 */
		public function ClauseCoordinationRule()
		{
			super();
		}
					...
					&& !PhraseChecker.expletiveSubjects(previous, next))
			{
						...
						&& PhraseChecker.samePostMods(previous, next))
				{
							...
							&& PhraseChecker.sameVPModifiers(previous, next))
					{
						...
						vp1 = previous.getFeatureAsElement(InternalFeature.VERB_PHRASE);
						...
						vp1= previous.getFeatureAsElement(InternalFeature.VERB_PHRASE);
						vp2 = next.getFeatureAsElement(InternalFeature.VERB_PHRASE);
						...
						&& PhraseChecker.samePostMods(previous, next))
				{
					...
					var subjects:CoordinatedPhraseElement = this.factory.createCoordinatedPhrase();
					subjects.setCategory(PhraseCategory.NOUN_PHRASE);
					var allSubjects:Array = previous.getFeatureAsElementList(InternalFeature.SUBJECTS);
					allSubjects = allSubjects.concat(next.getFeatureAsElementList(InternalFeature.SUBJECTS));
	
					var len:int = allSubjects.length;
					for (var i:int = 0; i < len; i++)
					{
						var subj:NLGElement = allSubjects[i];
